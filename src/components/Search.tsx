			...
			<div className='relative z-10 mt-8 max-w-xl w-full text-pink-700 rounded-xl flex items-center space-x-2 md:space-x-3'>
				...
				<div className='flex items-center space-x-2 sm:space-x-3 h-full'>
					{icons.map(({ id, Component }) => (
						<div
							key={id}
							className='bg-gradient-to-l duration-300 px-4 py-3 rounded-xl flex items-center cursor-pointer hover:scale-105 active:scale-95 border border-pink-200 bg-white shadow-xl'>
							<Component size={21} className='text-pink-800' />
						</div>
					))}
				</div>
			</div>
